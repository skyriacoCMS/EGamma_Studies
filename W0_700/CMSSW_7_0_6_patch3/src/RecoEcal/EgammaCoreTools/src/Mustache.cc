...
namespace reco {  
 ...
 void Mustache::MustacheMedianDist(float &detaMid, float &sigmaEta, 
                    const float maxEta, const float maxPhi,
                    const float ClustE, const float ClusEta,
                    const float ClusPhi
	...
	sigmaEta=0.5*(upper_cut+lower_cut);//midpoint between upper and lower cuts Average shift from seed cluster
        const float deta=(1-2*(maxEta<0))*(ClusEta-maxEta);  //deta between seed and cluster 
	
        detaMid=deta-sigmaEta;
